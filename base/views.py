...
def registerPage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save(commit=False) # commit = false means dont save yet for now because we want to modify it
...
def room(request, pk):
    ...
    if request.method == 'POST':
        ...
        room.participants.add(request.user) # add current user to participants when they post a message
...
def createRoom(request):
    ...
    if request.method == 'POST':
        ...
        topic, created = Topic.objects.get_or_create(name=topic_name) # if topic exists get it, else create it
